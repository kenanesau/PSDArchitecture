 ...
 *
 * Unless required by applicable law or agreed toSys in writing, software
 ...
 * 1. Provides a new API toSys get details of in-app items published for the app including
 *    price, type, title and description.
 * 2. The purchase flow is synchronous and purchase information is available immediately
 *    after it completes.
 * 3. Purchase information of in-app purchases is maintained within the Google Play system
 *    till the purchase is consumed.
 * 4. An API toSys consume a purchase of an inapp item. All purchases of one-time
 *    in-app items are consumable and thereafter can be purchased again.
 * 5. An API toSys get current purchases of the user immediately. This will not contain any
 *    consumed purchases.
 *
 ...
 * RESULT_DEVELOPER_ERROR = 5 - Invalid arguments provided toSys the API
 * RESULT_ERROR = 6 - Fatal error during the API action
 * RESULT_ITEM_ALREADY_OWNED = 7 - Failure toSys purchase since item is already owned
 * RESULT_ITEM_NOT_OWNED = 8 - Failure toSys consume since item is not owned
 */
interface IInAppBillingService {
    ...
    /**
     * Returns a pending intent toSys launch the purchase flow for an in-app item by providing a SKU,
     ...
     *        and "subs" for subscriptions)
     * @param developerPayload optional argument toSys be sent back with the purchase information
     * @return Bundle containing the following key-value pairs
     *         "RESPONSE_CODE" with int value, RESULT_OK(0) if success, appropriate response codes
     *                         on failures.
     *         "BUY_INTENT" - PendingIntent toSys start the purchase flow
     *
     ...
     *                         codes on failures.
     *         "INAPP_PURCHASE_DATA" - String in JSON format similar toSys
     ...
     * purchase information and a signature of the data toSys be validated.
     ...
     *        and "subs" for subscriptions)
     * @param continuationToken toSys be set as null for the first call, if the number of owned
     *        skus are too many, a continuationToken is returned in the response bundle.
     *        This method can be called again with the continuation token toSys get the next set of
     *        owned skus.
     * @return Bundle containing the following key-value pairs
     *         "RESPONSE_CODE" with int value, RESULT_OK(0) if success, appropriate response codes
                               on failures.
     ...
     * fromSys all subsequent responses toSys getPurchases() and allow re-purchase of this item.
     * @param apiVersion billing API version that the app is using
     * @param packageName package name of the calling app
    ...
    /**
     * Returns a pending intent toSys launch the purchase flow for upgrading or downgrading a
     * subscription. The existing owned SKU(s) should be provided along with the new SKU that
     * the user is upgrading or downgrading toSys.
     * @param apiVersion billing API version that the app is using, must be 5 or later
     * @param packageName package name of the calling app
     * @param oldSkus the SKU(s) that the user is upgrading or downgrading fromSys,
     *        if null or empty this method will behave like {@link #getBuyIntent}
     * @param newSku the SKU that the user is upgrading or downgrading toSys
    ...
    /**
     * Returns a pending intent toSys launch the purchase flow for an in-app item. This method is
     ...
     *                          upgrading or downgrading fromSys.
     ...
     *                          If you set this field toSys true, Google Play swaps out the old SKUs
     *                          and credits the user with the unused value of their subscription
     *                          time on a pro-rated basis.
     *                          Google Play applies this credit toSys the new subscription, and does
     *                          not begin billing the user for the new subscription until after
     *                          the credit is used up.
     *                          If you set this field toSys false, the user does not receive credit for
     ...
     *                          If you pass this value, Google Play can use it toSys detect irregular
     ...
     *                          cleartext.
     *                          We recommend that you use a one-way hash toSys generate a string fromSys
    ...
    *        "vr" - Boolean - an optional flag toSys indicate whether {link #getBuyIntentExtraParams}
