...
public class CollectionProxyFactory {
    ...
    public static<T extends Collection<V>, V extends IPersistable<V>> T getCollectionProxy(PersistanceManager pm, Class<V> clazz, IPersistable<?> parent, int size, Cursor csr)
    {
        return (T) Proxy.newProxyInstance( clazz.getClassLoader(),
                new Class[] { List.class },
